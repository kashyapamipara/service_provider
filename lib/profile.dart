...
class _ProfileState extends State<Profile> {
  ...
  Future<void> getData() async {
    ...
    for (int i = 0; i < allData.length; i++) {
      ...
      //   s += i.toString();
      // }
      if(allData[i]['admin_details_key'].toString() == widget.user_id) {
        username=allData[i]['name'];
        phone_number=allData[i]['phone'];
        email=allData[i]['email'];
      address=allData[i]['userAddress'];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body:isLoad?Center(child: CircularProgressIndicator(color: Colors.deepOrange,)) :SingleChildScrollView(
        child: Column(
          children: <Widget>[
            Padding(
              ...
              child: Container(
                ...
                child: Row(
                  ...
                  children: <Widget>[
                    ...
                    SizedBox(width: 50,),
                    Expanded(child: Text('${address}', style: TextStyle(fontWeight: FontWeight.bold , fontSize: 20),)),
                  ],
                ),
              ),
            ),
